...
class TvDatafeedLive(tvDatafeeds.TvDatafeed):
    """                 
    ...
    def new_seis(self, symbol, exchange, interval, timeout=-1): 
        ...
        new_seis=tvDatafeeds.Seis(symbol, exchange, interval)
    ...
    def new_consumer(self, seis, callback, timeout=-1):
        ...
        consumer=tvDatafeeds.Consumer(seis, callback)
        if self._lock.acquire(timeout=timeout) is False:
            return False
    ...
    def get_hist(self,  
        symbol: str,
        exchange: str = "NSE",
        interval: tvDatafeeds.Interval = tvDatafeeds.Interval.in_daily,
        n_bars: int = 10,
